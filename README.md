...
| **Limits** | Shared HF and Groq server keys: **30 successful generations per UTC day each**, keyed by browser id **and** IP. Your own keys are uncapped. Cache hits do not count. |
...
| Variable | Purpose |
| --- | --- |
| `HF_TOKEN` / `GROQ_API_KEY` | Server default keys (capped). Visitors can override in Settings. |
...
5. Default-key HF/Groq generations require Redis (503 if Redis is down) so a shared token cannot be burned without quotas.
