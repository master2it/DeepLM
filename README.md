...
1. Use the provider selected in Settings for grammar, tenses, and explain (no silent fallback to another vendor)
2. If no provider is sent, try Ollama (`think: false`), then Hugging Face (`Qwen/Qwen2.5-72B-Instruct`), then Groq (`openai/gpt-oss-120b`)
3. Strip leftover `<think>` blocks from replies

Keys are not required at startup. An explicit Groq/HF/Ollama choice fails if that provider is not configured or errors.
